...
export default async (req: Request) => {
  ...
  if (!apiKey) {
    return json({
      reply:
        "The AI assistant is ready to connect after this site’s first production deploy with Netlify AI Gateway enabled. Until then, try restating the constraint, walking through a tiny example, and checking edge cases (empty input, duplicates, overflow).",
    });
  }
