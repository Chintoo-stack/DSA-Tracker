import { FREE_QUESTION_COUNT, PLUS_QUESTION_COUNT, QUESTION_CATALOG, type Plan, type QuestionRecord } from "../data/catalog";
...
export function listDemoQuestions(plan: Plan) {
  const solved = new Set((loadDemo()?.solves ?? []).map((row) => row.questionId));
  const visible = plan === "plus" ? QUESTION_CATALOG : QUESTION_CATALOG.filter((q) => !q.isPlus);
  return visible.map((question) => ({ ...question, solved: solved.has(question.id) }));
}
...
export function demoSnapshot() {
  ...
  const catalog = plus ? QUESTION_CATALOG : QUESTION_CATALOG.filter((q) => !q.isPlus);
  const solvedIds = new Set(state.solves.map((row) => row.questionId));
  const solvedQuestionIds = catalog.filter((q) => solvedIds.has(q.id)).map((q) => q.id);
  const solvedDates = [
    ...new Set(state.solves.filter((row) => solvedIds.has(row.questionId) && catalog.some((q) => q.id === row.questionId)).map((row) => row.date)),
  ];
  return {
    user: state.user,
    progress: {
